...
func main() {
	log.SetOutput(ioutil.Discard) // Send all Log to nowhere by default

	log.AddHook(&writer.Hook{ // Send Log with level higher than warning to stderr
		Writer: os.Stderr,
		LogLevels: []log.Level{
			log.PanicLevel,
			log.FatalLevel,
	...
	log.AddHook(&writer.Hook{ // Send info and debug Log to stdout
		Writer: os.Stdout,
		LogLevels: []log.Level{
			log.InfoLevel,
			log.DebugLevel,
